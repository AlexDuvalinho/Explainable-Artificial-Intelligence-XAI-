...
# Store multiple categorical variables (more than 2 values)
...
X = telcom.iloc[:, 1: 32]  # delete customer ID from X because we won't use it to predict churn. 
X = X.drop(['Churn'], axis = 1) # create a dataframe with only independent variables. 
X = X.astype(float)   # keep a dataframe but where all columns are floats. 
Z = X.values  # Save X as an array under the name Z
y = telcom['Churn'].values   # define the dependent variable, as array. 
features = [i for i in telcom.columns if i not in Id_col + target_col] # store the name of all variables that we would like to use to build the model 
...
cm = confusion_matrix(y_test, y_pred, labels = [1, 0])  # reshape it so that 'churn' is the positive class. 
print(cm) # [211 170]
          #[ 98 928]


# Additional metrics to measure performance (f1, recall, precision, accuracy)
...
# Check the dataset distribution set according to the train/test split performed earlier. 
...
# Fit XGBoost to the Training set (not tuned for now). 
classifier = XGBClassifier()
classifier.fit(X_train, y_train)
...
visualizer.fit(X_train,y_train)
visualizer.poof()
# With SMOTE, the threshold is back to normal (approximatiely 0.5) so we won't set a particular threshold when building the model. 
...
# Fit the tuned XGBoost algorithm to the training set
...
print(cm) # [[271 110]
          # [204 822]]
# Note the trade off between False Positive and False Negative caused by the use of SMOTE. 
# A deeper analysis is proposed in the github read me document. 
